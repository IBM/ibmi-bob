...
PATH=/QOpenSys/pkgs/bin:/QOpenSys/usr/bin:/usr/bin time /QOpenSys/pkgs/bin/rpmbuild -ba --define '_topdir ~/rpmbuild' rpmbuild/SPECS/bob.spec
export RPM_FILE_PATH=$(find . -name '*.rpm' -print -quit)
echo "RPM ${RPM_FILE_PATH}"
mv ${RPM_FILE_PATH } bob.rpm