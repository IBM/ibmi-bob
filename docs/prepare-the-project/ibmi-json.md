...
* For each source stream file we don’t need the following
  ...
  * However this long name will affect include references in source and some customers may object to it being the name being changed, so it can be stored in a special comment beginning with %TEXT in the first 20 lines of the source
